...
set(_state_variable_names
	...
	GIT_COMMIT_DATE_UNIX
    GIT_AUTHOR_DATE_ISO8601
...
function(GetGitState _working_dir)
    ...
    if(exit_code EQUAL 0)
        set(ENV{GIT_COMMIT_DATE_UNIX} "${output}")
    endif()

    RunGitCommand(show -s "--format=%ai" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_AUTHOR_DATE_ISO8601} "${output}")
    endif()

    RunGitCommand(show -s "--format=%at" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_AUTHOR_DATE_UNIX} "${output}")
    endif()
    ...
    # Get output of git describe
    RunGitCommand(branch --show-current ${object})
    if(NOT exit_code EQUAL 0)
        set(ENV{GIT_BRANCH} "unknown")
    else()
...
function(SetupGitMonitoring)
    ...
    set_target_properties (check_git PROPERTIES FOLDER metalib/utils)
endfunction()
